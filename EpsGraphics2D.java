...
public class EpsGraphics2D extends java.awt.Graphics2D
{
  ...
  /** by tgh
   * Draws a bezier line from x0,y0 to x1,y1 with control points cx0,cy0 and cx1,cy1
   *
   */
  public void drawBezier(float x0_, float y0_, float cx0_, float cy0_, float cx1_, float cy1_, float x1_, float y1_) {
    ...
    for (int i=(int)(x0+1); i<=(int)x1; i++) {
      float t = map((float)i, x0, x1, 0.0f, 1.0f);
      float nx0 = (1f-t)*(1f-t)*(1f-t)*x0 + 3*(1f-t)*(1f-t)*t*cx0 + 3*(1f-t)*t*t*cx1 + t*t*t*x1;
      float ny0 = (1f-t)*(1f-t)*(1f-t)*y0 + 3*(1f-t)*(1f-t)*t*cy0 + 3*(1f-t)*t*t*cy1 + t*t*t*y1;
      
      t = map((float)(i-1), x0, x1, 0.0f, 1.0f);
      float nx1 = (1f-t)*(1f-t)*(1f-t)*x0 + 3*(1f-t)*(1f-t)*t*cx0 + 3*(1f-t)*t*t*cx1 + t*t*t*x1;
      float ny1 = (1f-t)*(1f-t)*(1f-t)*y0 + 3*(1f-t)*(1f-t)*t*cy0 + 3*(1f-t)*t*t*cy1 + t*t*t*y1;

      this.drawLine((int)nx0, (int)ny0, (int)nx1, (int)ny1);
    }
  }

  private float map(float v, float a0, float b0, float a1, float b1) {
    if (v < a0) {
      return a1;
    }
    else if (v > b0) {
